...
argument-hint: "[scan|action|recommend|patrol|trust|report|config|checkup] [args...]"
---
...
- **`recommend`** — Discover and recommend safe skills (AgentGuard-scanned)
...
Discover and recommend safe skills based on AgentGuard's 24-rule security scan. Only skills that pass all security checks (risk level LOW) are recommended.

### How It Works

1. **Collect** — Discovers candidate skills from local directories and registries
2. **Scan** — Runs AgentGuard's full 24-rule scan engine on each candidate
3. **Filter** — Only LOW-risk candidates pass; MEDIUM/HIGH/CRITICAL are eliminated
4. **Rank** — Scores candidates by relevance, domain match, and freshness
5. **Output** — Top recommendations with security attestation

### Usage

```
/agentguard recommend
/agentguard recommend --limit 5
/agentguard recommend --json
```
...
|------|---------|-------------|
| `--limit N` | 3 | Number of recommendations to show |
| `--min-score N` | 0.6 | Minimum composite score (0-1) |
| `--json` | false | Output as JSON |

### Steps

1. Resolve the skill directory (see "Resolving Script Paths" above)
2. Run the recommend script:
   ```bash
   cd <skill_directory> && node scripts/recommend.js [--limit N] [--json]
   ```
3. Parse the output and present to the user

### Output Format

```
## AgentGuard Skill Recommendations

Based on AgentGuard's security scan — only skills that pass all 24 detection rules:

  📡 【skill-name】
     Description of the skill
     安全状态：✅ LOW（0 项发现）
     推荐分：██████████ 0.85
     推荐原因：AgentGuard 扫描通过；涉及领域：security、devops
     安装：/skills install <identifier>
```

### When to Use

- User asks "what skills should I install" or "recommend a skill"
- User wants to discover new capabilities for their agent
- After `/agentguard checkup` if user asks for improvement suggestions
- Proactively during SessionStart when risky skills are detected (auto-recommend safer alternatives)
